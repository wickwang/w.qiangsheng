...
/**
 * Base class that represents a row from the 'trans_unit' table.
 *
 * 
 *
...
abstract class BaseTransUnit extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the msg_id field.
	 * @var        int
	 */
	protected $msg_id;
	...
	/**
	 * Initializes internal state of BaseTransUnit object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Set the value of [msg_id] column.
	 * 
	 * @param      int $v new value
	 * @return     TransUnit The current object (for fluent API support)
	 */
	...
	public function setDateAdded($v)
	{
		...
		if ($this->date_added !== $v || $this->isNew()) {
			$this->date_added = $v;
	...
	/**
	 * Set the value of [translated] column.
	 * 
	 * @param      int $v new value
	 * @return     TransUnit The current object (for fluent API support)
	 */
	public function setTranslated($v)
	{
		...
		if ($this->translated !== $v || $this->isNew()) {
			$this->translated = $v;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 10; // 10 = TransUnitPeer::NUM_COLUMNS - TransUnitPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	/**
	 * Generic method to set the primary key (msg_id column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of TransUnit (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setNew(true);
		$copyObj->setMsgId(NULL); // this is a auto-increment column, so set to default value
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     TransUnit Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseTransUnit:' . $name))
		{
