...
abstract class BaseConfigPeer {
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseConfigPeer', $criteria, $con);
		}
	...
	public static function populateObject($row, $startcol = 0)
	{
		...
		if (null !== ($obj = ConfigPeer::getInstanceFromPool($key))) {
			...
			$col = $startcol + ConfigPeer::NUM_COLUMNS;
		} else {
	...
	{
		return $withPrefix ? ConfigPeer::CLASS_DEFAULT : ConfigPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a Config or Criteria object.
	 *
	...
	/**
	 * Method perform an UPDATE on the database, given a Config or Criteria object.
	 *
	...
	/**
	 * Method to DELETE all rows from the config table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a Config or Criteria object OR a primary key value.
	 *
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseConfigPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
