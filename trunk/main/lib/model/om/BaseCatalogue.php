...
/**
 * Base class that represents a row from the 'catalogue' table.
 *
 * 
 *
...
abstract class BaseCatalogue extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseCatalogue object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Catalogue The current object (for fluent API support)
	 */
	...
	public function setTypeId($v)
	{
		...
		if ($this->type_id !== $v) {
			$this->type_id = $v;
			$this->modifiedColumns[] = CataloguePeer::TYPE_ID;
		}
	...
	public function setSourceLang($v)
	{
		...
		if ($this->source_lang !== $v || $this->isNew()) {
			$this->source_lang = $v;
	...
	public function setTargetLang($v)
	{
		...
		if ($this->target_lang !== $v || $this->isNew()) {
			$this->target_lang = $v;
	...
	public function setDateCreated($v)
	{
		...
		if ($this->date_created !== $v || $this->isNew()) {
			$this->date_created = $v;
	...
	public function setDateModified($v)
	{
		...
		if ($this->date_modified !== $v || $this->isNew()) {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->source_lang = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->target_lang = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->date_created = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->date_modified = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			...
			return $startcol + 9; // 9 = CataloguePeer::NUM_COLUMNS - CataloguePeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CataloguePeer::CAT_ID) ) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getSourceLang(),
			$keys[5] => $this->getTargetLang(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setSourceLang($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setTargetLang($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CataloguePeer::TYPE_ID)) $criteria->add(CataloguePeer::TYPE_ID, $this->type_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Catalogue (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Catalogue Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseCatalogue:' . $name))
		{
