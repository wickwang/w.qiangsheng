...
//    $categories = self::getProperty('categories', array());
    $categories = AdminCategoryPeer::getCategoriesForsfAdminDash();
  ...
  */
  ...
  public static function getCurrentCategoryAndItemByLink(){
    $uri = $_SERVER['REQUEST_URI'];
    $uri_parts = explode('?',$uri);
    $uri = isset($uri_parts[0]) ? $uri_parts[0] : '';
      
    $current_link_parts = explode('/', $uri);
    $current_link = isset($current_link_parts[2]) ? $current_link_parts[2] : '';

    $my_category = null;  
    $my_item = null;  
    
    foreach (sfAdminDash::getCategories() as $category_name=>$category) {
        if(isset($category['items'])){
            
            foreach ($category['items'] as $item) {
                $parts = explode('/',$item['url']);
                $item_module_link = isset($parts[2]) ? $parts[2] : '';
                
                if ($item_module_link == $current_link){
    ...
    return array($my_category, $my_item);           
  }
